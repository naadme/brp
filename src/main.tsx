...
function App(){
 ...
 const sorted=useMemo(()=>[...situations].sort((a,b)=>view==='trending'?b.hot-a.hot:b.advice.reduce((n,x)=>n+x.votes,0)-a.advice.reduce((n,x)=>n+x.votes,0)),[situations,view]); const topicTags:Record<string,string[]>={Love:['ROMANCE'],Work:['WORK'],Friendship:['FRIENDS'],Family:['FAMILY'],Dating:['ROMANCE'],Money:['WORK'],College:['WORK'],Life:['OTHER'],Wellness:['OTHER'],Roommates:['HOUSE']}; const filtered=topic==='All'?sorted:sorted.filter(s=>(topicTags[topic]||['OTHER']).includes(s.tag)); const pageCount=Math.max(1,Math.ceil(filtered.length/6)); const visible=filtered.slice((page-1)*6,page*6);
...
function TopicBar({active,onSelect}:{active:string;onSelect:(topic:string)=>void}){const [more,setMore]=useState(false);const topics=['All','Love','Work','Friendship','Family','Dating','Money','College','Wellness','Roommates'];const moreTopics=['Anxiety','Career switch','Coworkers','Breakups','Moving out','Fitness','Sleep','Travel','Pets','Food','Style','Gaming','Music','Movies','Books','Art','Photography','Social media','Dating apps','First date','Long distance','Marriage','Divorce','Parenting','Siblings','In-laws','Neighbors','Landlord','Rent','Bills','Debt','Saving','Investing','Side hustle','Startup','Freelance','Job interview','Burnout','Office politics','Remote work','Study tips','Exams','University','School','Teachers','Language','Confidence','Self-esteem','Habits','Productivity','Time management','Motivation','Meditation','Therapy','Health','Cooking','Home decor','Cleaning','Tech','Phones','Coding','AI','Privacy','Cars','Commuting','Sports','Fashion','Skincare','Hair','Weekend plans','Parties','Hosting','Gifts','Birthdays','Holidays','Culture','Faith','Identity','Community','Volunteering','Environment','News','Politics','Legal basics','Customer service','Small business','Networking','Public speaking','Creative block','Pet peeves','Apologies','Boundaries','Conflict','Jealousy','Trust','Loneliness','Grief','Big decisions','Life advice','Just venting'];return <div className="topic-wrap"><div className="topic-bar" aria-label="Browse topics">{topics.map(topic=><button key={topic} className={active===topic?'selected':''} onClick={()=>onSelect(topic)}>{topic}</button>)}<button className={more?'selected more-trigger':'more-trigger'} onClick={()=>setMore(!more)}>Other {more?'−':'+'}</button></div>{more&&<div className="more-topics"><div className="more-topics-head"><b>More topics</b><span>{moreTopics.length} ways to start a conversation</span></div><div className="more-topic-grid">{moreTopics.map(topic=><button key={topic} className={active===topic?'selected':''} onClick={()=>{onSelect(topic);setMore(false)}}>{topic}</button>)}</div></div>}</div>}
